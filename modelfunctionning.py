...
def generate_pdf(session_data):
    ...
    pdf.cell(200, 10, txt="Career Recommendation Session Summary", ln=True, align="C")
    ...
    for key, value in session_data.items():
        ...
        if isinstance(value, list):
            ...
            for sub_key, sub_value in value.items():
                pdf.cell(0, 10, txt=f"  {sub_key}: {sub_value}", ln=True)
        else:
...
def dynamic_translate(text, target_language):
    ...
    try:
        response = openai.ChatCompletion.create(
            model="gpt-4o",  # Use "gpt-4" if you have access and prefer
            messages=[
                {
                    "role": "system",
...
def download_button(data, filename, label):
    ...
    Args:
        data (bytes): PDF content as bytes.
...
def create_career_roadmap(user_skills, target_skills, milestones):
    # Create a Graphviz graph
    career_roadmap = graphviz.Digraph(format='png')

    # Add user skills (Current Skills)
    career_roadmap.node('A', 'Current Skills')
    for skill in user_skills:
        career_roadmap.node(skill, skill)
        career_roadmap.edge('A', skill)

    # Add required skills (Skills needed for next steps)
    career_roadmap.node('B', 'Required Skills')
    for skill in target_skills:
        career_roadmap.node(skill, skill)
        career_roadmap.edge('B', skill)

    # Add milestones between skills
    for milestone in milestones:
        career_roadmap.node(milestone, milestone)
        career_roadmap.edge('A', milestone)
        career_roadmap.edge(milestone, 'B')

    # Display the graph using Streamlit
    st.graphviz_chart(career_roadmap)

# Function to plot career roadmap with Plotly
def plot_career_roadmap(user_skills, target_skills, milestones):
    all_nodes = user_skills + milestones + target_skills
    edges = []

    # Create edges from current skills to milestones
    for skill in user_skills:
        for milestone in milestones:
            edges.append((skill, milestone))

    # Create edges from milestones to required skills
    for milestone in milestones:
        for skill in target_skills:
            edges.append((milestone, skill))

    # Create a Plotly figure
    fig = go.Figure(data=[go.Scatter(
        x=[1, 2, 3],  # Example coordinates for nodes
        y=[1, 2, 3],
        mode='markers+text',
        text=all_nodes,
    ...
    for edge in edges:
        fig.add_trace(go.Scatter(
            x=[edge[0], edge[1]],  # X coordinates of the edge (start, end)
            y=[edge[0], edge[1]],  # Y coordinates of the edge (start, end)
            mode='lines+text',
    ...
    fig.update_layout(title='Career Roadmap', showlegend=False)
    st.plotly_chart(fig)
...
""", unsafe_allow_html=True)
    ...
    navigation = st.sidebar.radio(
        "Choose a section:",
        ["Home", "Upload CV", "View Recommendations", "Feedback & Refinement", "Generate and Track SMART Goals", "Motivational Quotes", "Interactive Career Roadmap"]
    )
    ...
    elif navigation=="Interactive Career Roadmap":
        st.title('Interactive Career Roadmap')

        # Input fields for user skills, target skills, and milestones
        user_skills = st.text_input('Enter your current skills (comma-separated)', 'Python, Data Analysis').split(', ')
        target_skills = st.text_input('Enter the required skills for your next step (comma-separated)', 'Deep Learning, AI Research').split(', ')
        milestones = st.text_input('Enter milestones (comma-separated)', 'Complete Deep Learning Course, Build AI Project').split(', ')

        # Choose between Graphviz or Plotly visualization
        chart_type = st.radio("Choose roadmap visualization", ('Graphviz', 'Plotly'))

        if chart_type == 'Graphviz':
            create_career_roadmap(user_skills, target_skills, milestones)
        else:
            plot_career_roadmap(user_skills, target_skills, milestones)
